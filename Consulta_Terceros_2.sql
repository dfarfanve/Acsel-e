...
spool \\10.54.66.107\adn\ACSEL\ConsultasCONSIS\Consulta_Terceros_2.csv

--SIN NOT NULL
--Muestra todos los registros para los terceros repetidos por CLIENTEUNICOTDAINPUT y que tengan polizas asociadas (aplicadas)
...
WHERE EXISTS(
              select NP.CLIENTEUNICOTDAINPUT
              from AZTECA_V138.NATURALPERSON NP
              WHERE NP.CLIENTEUNICOTDAINPUT = NP1.CLIENTEUNICOTDAINPUT
              ...
              GROUP BY NP.CLIENTEUNICOTDAINPUT
              HAVING COUNT(NP.CLIENTEUNICOTDAINPUT) > 1
          );
---
