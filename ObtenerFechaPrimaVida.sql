...
where opr.policyid = info.item and opm.dty_id = 7572 and opm.status in ('active','applied')) last_opm from
  ( select pp.numeropolizainput,
      ...
      --ctx.time_Stamp,
      POL.PK AS CLAVE,
      'UPDATE INTERSEGURO.POLEDUCACIONALSURA SET FECHAPROXGENPRIMAINPUT = TO_DATE ('''|| MIGRATION_UTILS.getNextPremiumGenerationDate(pd.INITIALDATE , ctx.TIME_STAMP ,1 , 12) ||''', ''YYYY-MM-DD'') WHERE PK = ' || PP.PK || ';' AS QUERY
    from interseguro.prepolicy pp
      inner join interseguro.policydco pd on pd.dcoid = pp.pk
      inner join INTERSEGURO.POLEDUCACIONALSURA pol on pol.pk = pp.pk
      ...
      --inner join INTERSEGURO.POLVIDAENTERA pol on pol.pk = pp.pk
      --inner join INTERSEGURO.POLEDUCGARANTIZADANEW pol on pol.pk = pp.pk
      ...
      inner join interseguro.UREDUCACIONALSURA ur on ur.pk = rdco.dcoid
    ...
    where pp.numeropolizainput IN ('63672')
    order by ctx.time_Stamp desc
  ) info where next <>  FECHAPROXGENPRIMAVIDAINPUT-- ;

SELECT PK_UTIL.DAYS_DIFF('2019-10-01') FROM DUAL;